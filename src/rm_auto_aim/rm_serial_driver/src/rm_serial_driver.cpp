...
RMSerialDriver::RMSerialDriver(const rclcpp::NodeOptions &options)
    : Node("rm_serial_driver", options), owned_ctx_{new IoContext(2)},
      serial_driver_{new drivers::serial_driver::SerialDriver(*owned_ctx_)} {
    RCLCPP_INFO(get_logger(), "Start RMSerialDriver!");
    if (!getParams())
        RCLCPP_WARN(get_logger(), "Can't open config!");
    ...
    marker_pub_ = this->create_publisher<visualization_msgs::msg::Marker>(
        "/aiming_point", 10);
    bool port_init_ok = false;
    size_t port_init_false_count = 0;
    while (!port_init_ok && rclcpp::ok()) {
        try {
            serial_driver_->init_port(device_name_, *device_config_);
            if (!serial_driver_->port()->is_open()) {
                serial_driver_->port()->open();
                receive_thread_ =
                    std::thread(&RMSerialDriver::receiveData, this);
                port_init_ok = true;
            }
        } catch (const std::exception &ex) {
            port_init_ok = false;
            ++port_init_false_count;
            RCLCPP_ERROR(get_logger(),
                         "Error creating serial port %s false %lu times:%s",
                         device_name_.c_str(), port_init_false_count,
                         ex.what());
    ...
    // aiming_point_.header.frame_id = "camera_optical_frame";
        ...
        try {
            ...
            if (header[0] == 0xA5) {
                ...
                if (packet.ender == 0xFF) {
                    ...
                    RCLCPP_WARN(get_logger(), "Ender is %02X error!",
                                packet.ender);
                }
            } else {
...
void RMSerialDriver::sendData(
    ...
    try {
        SendPacket packet;
        packet.pitch =
            atan(msg->position.y / hypot(msg->position.x, msg->position.z));
        packet.yaw =
            atan(msg->position.x / hypot(msg->position.y, msg->position.z));
        packet.distance =
            hypot(hypot(msg->position.x, msg->position.y), msg->position.z);
        packet.is_find_target = msg->tracking;
        packet.is_middle = (abs(packet.pitch) <= 0.01) &&
                           (abs(packet.yaw) <= 0.01) && packet.is_find_target;
        // aiming_point_.pose.position;
        aiming_point_.header.stamp = this->now();
        aiming_point_.pose.position.x = packet.distance;
        aiming_point_.pose.position.y = packet.yaw;
        aiming_point_.pose.position.z = packet.pitch;
        marker_pub_->publish(aiming_point_);
        ...
        RCLCPP_DEBUG_STREAM(get_logger(),
                            "Total latency: " + std::to_string(latency.data) +
                                "ms");
