...
bool auto_aim_interfaces__msg__target__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // z_2
    PyObject * field = PyObject_GetAttrString(_pymsg, "z_2");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->z_2 = PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
...
PyObject * auto_aim_interfaces__msg__target__convert_to_py(void * raw_ros_message)
{
  ...
  {  // z_2
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->z_2);
    {
      int rc = PyObject_SetAttrString(_pymessage, "z_2", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
